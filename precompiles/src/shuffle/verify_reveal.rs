...
impl VerifyReveal {
    ...
    pub fn new(data: &[u8]) -> Result<Self> {
        ...
        let reveal_token = utils::into_bytes(res.get(2).cloned()).ok_or(Error::ParseDataFailed)?;
        let masked = utils::into_bytes(res.get(3).cloned()).ok_or(Error::ParseDataFailed)?;
        let reveal_proof = utils::into_bytes(res.get(4).cloned()).ok_or(Error::ParseDataFailed)?;
