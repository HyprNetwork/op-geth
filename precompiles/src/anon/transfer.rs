...
pub struct Transfer {
    ...
    root: Vec<[u8; 32]>,
    transparent_account: Vec<Address>,
...
impl Transfer {
    ...
    pub fn new(data: &[u8]) -> Result<Self> {
        ...
        let transparent_accounts = tokens
            .get(6)
            .and_then(|v| v.clone().into_array())
            .and_then(|vs| {
                vs.iter()
                    .map(|v| utils::into_address(Some(v.clone())))
                    .collect::<Option<Vec<_>>>()
            })
            .ok_or(Error::ParseDataFailed)?;
        ...
        let r = Self {
            ...
            root: roots,
            transparent_account: transparent_accounts,
    ...
    pub fn require(&self) -> Result<()> {
        ...
        if self.commitments.len() == length
            ...
            && self.transparent_account.len() == length
